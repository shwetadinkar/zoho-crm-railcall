...
Run `zoho.verify_connection` first. It probes every scope and names any that are missing and which commands they block, so a bad setup surfaces immediately rather than three commands later.
...
100 records per write call, 2000 per scan before it refuses rather than half-reporting. Deletes go to the recycle bin, recoverable 60 days; rollback covers updates, not deletes. Bulk and Notification APIs are not wrapped. Activities, attachments and tags are not covered. The manifest declares `subprocess: false`; enforcement is at the Python import layer, not a container.
